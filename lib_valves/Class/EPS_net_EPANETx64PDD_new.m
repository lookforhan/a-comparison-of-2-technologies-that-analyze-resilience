    ...
    methods
        ...
        function Run_debug(obj) %延时模拟，计算时间段内所有时间步
            ...
            while (temp_tstep && ~errcode4)
                time_step_n = time_step_n+1;
                % check pipe statu
                value = libpointer('doublePtr',100);count= libpointer('int32Ptr',1);
                [code,count] = calllib(obj.lib_name,'ENgetcount',2,count);
               id = libpointer('cstring','a');
                for mm = 1:count
                   [code, pipe_statu(mm,1)] = calllib(obj.lib_name,'ENgetlinkvalue',mm,11,value);
                   [code,pipe_id{mm,1}] = calllib(obj.lib_name,'ENgetlinkid',mm,id);
                end
                if errcode4
                    keyboard
                end
                    ...
                    for i = 1:numel(mid_status)
                        ...
                        switch  pipe_status_timeStep
                            ...
                            case 1
                                ...
                                str2(str2_n:str2_n+str_length-1) = mid_str;
                                str2_n = str2_n+str_length;
                                [~,the_pipe_segment_loc] = ismember(isolated_pipe{i,1},obj.pipe_id);
                            ...
                            case 2
                                ...
                                %reopen
                                isolated_pipe_new = isolated_pipe;
                                isolated_pipe_new(cellfun(@isempty,isolated_pipe_new)) = [];
                                if ismember(obj.pipe_relative{i,1},isolated_pipe_new)
                                ...
                                str3(str3_n:str3_n+str_lenth-1) = mid_str_2;
                                str3_n = str3_n+str_lenth;
                                the_pipe = obj.pipe_relative{i,1};
                                the_pipe_segment_loc = ismember(the_pipe,obj.pipe_id);
                                the_pipe_segment = obj.Pipe_flag(the_pipe_segment_loc);
                                the_segment_pipe_close = obj.Valve_segment(the_pipe_segment);
                                for p = 1:numel(the_segment_pipe_close) % 所处区域内管道均关闭
                                    id = libpointer('cstring',the_segment_pipe_close{p,1}{1});
                                    index =libpointer('int32Ptr',0);
                                    [code,id,index]=calllib(obj.lib_name,'ENgetlinkindex',id,index);
                                    code=calllib(obj.lib_name,'ENsetlinkvalue',index,11,1);%管道id状态为关闭
                                end
%                                 id=libpointer('cstring',obj.pipe_relative{i,1});
%                                 index =libpointer('int32Ptr',0);
%                                 [code,id,index]=calllib(obj.lib_name,'ENgetlinkindex',id,index);
                                ...
                                the_pipe = obj.pipe_relative{i,1};
                                id = libpointer('cstring',the_pipe);
                                index =libpointer('int32Ptr',0);
                                [code,id,index]=calllib(obj.lib_name,'ENgetlinkindex',id,index);
                                code= calllib(obj.lib_name,'ENsetlinkvalue',index,11,1);
                                if code
