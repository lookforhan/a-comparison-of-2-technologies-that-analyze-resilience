...
%% example
function [PipeStatus,PipeStatusChange]=schedule2pipestatus5_isolation(BreakPipe_result,Damage_pipe)
...
n_recovery=numel(BreakPipe_RepairResult(:,1));%破坏管道个数
nB = numel(Damage_pipe);
PipeStatus_original=2*ones(nB,1);%管道初始状态为破坏中
PipeStatusChange_original = zeros(nB,1);
endTime=ceil(max(cell2mat(BreakPipe_InspectResult(:,4))));%最大的修复完成时间为时间步终止
...
for i_replacement= 1:n_recovery
...
for i = 1:nB
    if time_1(i) ==0
        ...
        PipeStatus_1(i,time_1(i):end) = 1;%隔离,修复时管道状态均为隔离
