...
%}
function [newpop,best_indiv]=selection2(pop,Fitvalue)
popsize=numel(pop);
...
% max_fitness=fitvalue2(index1);
best_indiv=pop(index1);
[~,index2]=min(fitvalue);%最差个体适应度值
index=1:popsize;index(index1)=0;index(index2)=0;%删除掉最优和最差个体的位置
index=nonzeros(index);%删除掉最优和最差个体的位置
evo_pop=pop(index);%删除掉最优和最差个体的种群
evo_fitvalue=fitvalue(index);%删除掉最优和最差个体的适应度
