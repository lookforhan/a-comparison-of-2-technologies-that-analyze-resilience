% 新建一个变量，记录模拟过的个体，并比较
function [pop_record,Fit_record,generation_result,fitvalue_result_record]...
    =ga_priority6(popsize,generation_length,pc,pm,rate_best_pop,ga_doa,...种群大小，进化代数，交叉概率，变异概率
...
Fit_record = Fit_uniq;
fitvalue_result = fitvalue(ic1,:);
fitvalue_result_record=fitvalue;
else
    Fit_init_1=cell(0,1);
    ...
    fitvalue_result_1=cell(0,15);
    fitvalue_result_1(uniq_loc_in_pop,:) = fitvalue;
    fitvalue_result_1(not_uniq_loc_in_pop,:) = fitvalue_result_record(not_uniq_loc_in_record,:);
    fitvalue_result = fitvalue_result_1(uniq1_loc_in_init,:);
    fitvalue_result_record=[fitvalue_result_record;fitvalue];
end
generation_result{generation_i,1}= fitvalue_result;
