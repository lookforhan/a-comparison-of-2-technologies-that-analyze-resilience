...
function [results]...
    =ga_priority10_epanetx64pdd(popsize,generation_Nmax,pc,pm,...种群大小，进化代数，交叉概率，变异概率
    out_dir,....输出目录
    RepairCrew,...修复队伍，
...
for generation_i = 1:generation_Nmax
    ...
    if ~isempty(pop_isolation_uniq)
        [Fit_isolation_uniq,~]=calfitvalue4_isolation(pop_isolation_uniq,damage_pipe_info{1},RepairCrew,...
...
for generation_i = 1:generation_Nmax
    ...
    if ~isempty(pop_recovery_uniq)
        [Fit_recovery_uniq,~]=calfitvalue4_recovery(best_indivi_isolation,pop_recovery_uniq,DamagePipe_order,RepairCrew,...
     Dp_Inspect_mat,Dp_Repair_mat,Dp_Travel_mat,...
     crewStartTime,crewEfficiencyRecovery,crewEfficiencyIsolation,crewEfficiencyTravel,...
    damage_pipe_info,net_data,...
    EPA_format,...
...
[ ~,~,~,~, system_serviceability_cell,activity_cell] = EPS_net4_EPANETx64PDD( output_net_filename_inp,output_result_dir,PipeStatus,PipeStatusChange,pipe_relative,net_data,...
    duration_one);
