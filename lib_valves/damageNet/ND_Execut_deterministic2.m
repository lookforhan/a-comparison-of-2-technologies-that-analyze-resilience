...
% [18]L1,破坏长度（m）
%%
function [t,damage_pipe_info]=ND_Execut_deterministic2(net_data,pipe_damage_data,C,mu,pipe_damage_num_max)
pipe_id=net_data{5,2}(:,1); %管线的原始编号
damage_num=numel(pipe_damage_data{1}); %破坏点的数量；
damage_pipe_id=pipe_damage_data{2}; %破坏所在管线编号；
...
damage_node_area=pipe_damage_data{6}; %破坏点的开口面积；
...
D2=zeros(damage_pipe_num,pipe_damage_num_max); %每个破坏管道中的破坏点长度位置；
D3=zeros(damage_pipe_num,pipe_damage_num_max); %每个破坏管道中的破坏点的类型；1渗漏/2断开；
D4=zeros(damage_pipe_num,pipe_damage_num_max); %每个破坏管道中的破坏点的开口面积；渗漏按实际计算,断开按截面积计算；
...
[~,M1]=ismember(D1,pipe_id);%每个破坏管道ID号在所有管线中的排序号；
mid_D2=1-sum(D2,2);
