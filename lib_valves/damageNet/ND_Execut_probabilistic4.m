...
damage_num=numel(pipe_damage_data{1,1}); %破坏点的数量；
damage_pipe_id=deblank(pipe_damage_data{1,2}); %破坏所在管线编号；
damage_pipe_num=numel(unique(damage_pipe_id));%含有破坏点的管道数量；
damage_pipe_node=pipe_damage_data{1,3}; %破坏点在该管线上的破坏次序；
damage_pipe_length=pipe_damage_data{1,4}; %破坏点与点之间的长度比例；
damage_node_kind=pipe_damage_data{1,5}; %破坏点的类型；
damage_node_area=pipe_damage_data{1,6}; %破坏点的开口面积（m2）
