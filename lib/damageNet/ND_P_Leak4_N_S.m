...
% [7]c1,cell,
%%
function [t,all_add_node_data,pipe_new_add_new]=ND_P_Leak4_N_S(damage_node_data,damage_pipe_info,pipe_new_add)
...
%         pipe_new_add_2(n3,1).Length=1;%管道长度为1m
%         pipe_new_add_2(n3,1).Roughness=damage_node_data_solo{i}.Roughness;
%         pipe_new_add_2(n3,1).MinorLoss=0;
...
% all_add_node_data=cell(damage_node_num+break_node_num+n3,numel(mid_data));
  all_add_node_data=cell(damage_node_num+break_node_num,numel(mid_data));  
%----------------------------------------------------------
%结构体数据中间转换——原破坏点数据
