...
% t
% outdata 水力模型文件
function [outdata]=timeStepNet2(PipeStatus_timeStep,damage_pipe_info,net_data)
% 初始化数据
...
%     disp('没有破坏')
    ...
    %% 生成破坏后管网信息
    [t_j,damage_node_data]=ND_Junction5(net_data,damage_pipe_info_after);%生成管线中的破坏点数据
...
%         disp('ND_Junctions出错！')
    ...
    [t_p,pipe_new_add]=ND_Pipe5(damage_node_data,damage_pipe_info_after,net_data{5,2});%生成管线破坏点的邻接管段数据
...
%         disp('ND_Pipes出错！')
    ...
    [t,all_add_node_data,pipe_new_add]=ND_P_Leak4_GIRAFFE2_R(damage_node_data,damage_pipe_info_after,pipe_new_add);
