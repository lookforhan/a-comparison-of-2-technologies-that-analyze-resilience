...
function [Fitness,best_pop,BreakPipe_result,RepairCrew_result,...
    ...
    y,y1,x1]...
    =ga_priority2(popsize,generation_length,pc,pm,rate_best_pop,...种群大小，进化代数，交叉概率，变异概率
...
%     )%
best_pop_num = ceil(popsize*rate_best_pop);
 
        [pop]=initpop(popsize,BreakPipe_order);%生成初始种群---0104版本采用initpop2.m
        best_pop=cell(best_pop_num,generation_length);%?
